...
namespace DragonsDecorativeMod.Content.Tiles.Garden
{
    public class SingleTilePlant : ModTile
    {
        public override void SetStaticDefaults()
        {
            ...
            TileObjectData.newTile.CoordinateHeights = [48];
        ...
        public override IEnumerable<Item> GetItemDrops(int i, int j)
        {
            ...
            if (style < 2)
            {
                yield return new Item(ModContent.ItemType<Items.Placeable.Garden.SingleTilePlant>());
            }
            else if (style < 4)
            {
                yield return new Item(ModContent.ItemType<Items.Placeable.Garden.SingleTilePlant2>());
            }
            else if (style < 6)
            {
                yield return new Item(ModContent.ItemType<Items.Placeable.Garden.SingleTilePlant3>());
            }
            else
            {
                yield return new Item(ModContent.ItemType<Items.Placeable.Garden.SingleTilePlant4>());
            }
        }
    }
}
