...
namespace DragonsDecorativeMod.Content.Tiles.Garden
{
    public class Planter : ModTile
    {
        ...
        public override IEnumerable<Item> GetItemDrops(int i, int j)
        {
            ...
            if (style == 0)
            {
                yield return new Item(ModContent.ItemType<Items.Placeable.Garden.PlanterRound>());
            }
            else
            {
                yield return new Item(ModContent.ItemType<Items.Placeable.Garden.PlanterLarge>());
            }
        }
    }
}
