...
namespace DragonsDecorativeMod.Content.Tiles.Garden.PottedPlants
{
    public class PottedTrees : ModTile
    {
        ...
        public override IEnumerable<Item> GetItemDrops(int i, int j)
        {
            ...
            if (style == 0)
            {
                yield return new Item(ModContent.ItemType<Items.Placeable.Garden.PottedPlants.PottedTreeSnowy>());
            }

            if (style == 1)
            {
                yield return new Item(ModContent.ItemType<Items.Placeable.Garden.PottedPlants.PottedTreeCorruption>());
            }
            else if (style == 2)
            {
                yield return new Item(ModContent.ItemType<Items.Placeable.Garden.PottedPlants.PottedTreeCrimson>());
            }
        }
