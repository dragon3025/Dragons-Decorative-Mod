...
{
    public class BalloonsOnePaintableTwoPurple : ModTile
    {
        ...
        public override void SetStaticDefaults()
        {
            ...
            AddMapEntry(new Color(128, 0, 255));

            AnimationFrameHeight = 54;

            if (!Main.dedServ)
            {
                overlayTexture = ModContent.Request<Texture2D>("DragonsDecorativeMod/Tiles/TiedBalloons/BalloonsOnePaintableTwoPurpleOverlay");
            }
        }

        public override void KillMultiTile(int x, int y, int frameX, int frameY)
        {
            Item.NewItem(new EntitySource_TileBreak(x, y), x * 16, y * 16, 32, 32, ModContent.ItemType<Items.TiedBalloons.BalloonsOnePaintableTwoPurple>());
        }
